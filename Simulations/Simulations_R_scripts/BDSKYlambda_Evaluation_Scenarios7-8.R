...
read = 0
scenario = 1; 
scenarios=c("Scenario7", "Scenario8")
#Scenario7: 6 transmission trees, N=50, three transmission trees with lambda_ratio = 1.0 and three with lambda_ratio = 1.666.
#Scenario8: 6 transmission trees, N=10-50, three transmission trees with lambda_ratio = 1.0 and three with lambda_ratio = 1.6.
#For both scenarios change in R_base. 
#For scenario7 R_base_t1 = 3.0, R_base_t2 = 2.3
#For scenario8 R_base_t1 = 1.17, R_base_t2 = 0.9

#For both scenarios lambda_ratio and samplingProportion inferred jointly for trees with lambda_ratio_low and lambda_ratio_high.


#L=c(109.3,182.5) # For scenario 7
#L=c(42.7,68.3) # For scenario 8
...
for(scenario in 1:2){
  ...
  if (read==0){
    ...
    for(i in 1:dim){
      ...
      ess1 = effectiveSize(get(paste("log", i, sep=''))[round(burninperc*thissample):thissample,2:35]) # check all likelihood and parameter ESSs 
  ...
  if (scenario==1){Rb = c(3.0,2.3)
  s = c(0.0,0.01,0.0,0.01)
  l=c(1.0,1.666)
  pars = 8 #length(names)
  c = colnameindex(log1,'baseReproductiveNumber.t1')
  names = names(log1)[c:(c+1)]
  ...
  if (scenario==2){Rb = c(1.17,0.9)
  s = c(0.0,0.01,0.0,0.01)
  l=c(1.0,1.6)
  pars = 8 #length(names)
  c = colnameindex(log1,'baseReproductiveNumber.t1')
  names = names(log1)[c:(c+1)]
  c = colnameindex(log1,'lambda_ratio_42.7')
  names = c(names,names(log1)[c:(c+1)])
  c = colnameindex(log1,'samplingProportion_42.7.t1')
  names = c(names,names(log1)[c:(c+3)])
  }
  ...
  if (scenario == 1) {
    paramnames=c(expression(R[paste(base,",","t1")]),expression(R[paste(base,",","t2")]),expression(r[paste(lambda,",",1)]),expression(r[paste(lambda,",",2)]),expression(s[paste(1,",",1)]),expression(s[paste(1,",",2)]),expression(s[paste(2,",",1)]),expression(s[paste(2,",",2)]))
  } 
  else  if (scenario == 2) {
    paramnames=c(expression(R[paste(base,",","t1")]),expression(R[paste(base,",","t2")]),expression(r[paste(lambda,",",1)]),expression(r[paste(lambda,",",2)]),expression(s[paste(1,",",1)]),expression(s[paste(1,",",2)]),expression(s[paste(2,",",1)]),expression(s[paste(2,",",2)]))
  } 
